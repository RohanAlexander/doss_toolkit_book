...
After going through the previous chapters learning the tidyverse framework, we can start building various statistical models to incorporate to your analyses.
...
Note that the package loads some core `tidyverse` packages as well, including `dplyr`, `tidyr`, and `ggplot2`.    

We will focus on the core packages in `tidymodels`, including:   
- `rsample`  
- `recipes`  
- `parsnip`  
- `yardstick`  
...
`rsample` is a General Resampling Infrastructure for R so that models can be assessed and empirically validated. The package comes in handy when you want to separate a dataset into training dataset and testing dataset.   

**What is resample?**  
Resample is a result of a two-way split of a dataset.  
...
Using the same mtcars training dataset, you can create a recipe and prep it. You can use the pipe operator `%>%` like we used in the `dplyr` package when modeling.   
```
...
Then, by using the function `bake()`, you can execute the preprocessing using the testing data, as we did for the training data as the following.  
...
The `yardstick` package allows you to estimate how well models are performing using tidy data principles. For regression models, we can use R-squared or MSE to evaluate the model performnce, however classifier evaluation requires a little more than that. The package automates the complex calculation process.   

There are three main types of metrics in `yardstick`:   
1. Class metrics    
2. Class probability metrics    
3. Numeric metrics  

They all take `truth` and `estimate` as input arguments (input types vary) and return the same type of output (a tibble).  


Aside from the usage of the existing functions, you can also create custom metrics to evaluate your models.  
...
For more information on `yardstick`, here is an [additional resource on yardstick package](https://yardstick.tidymodels.org) you may find helpful.   
